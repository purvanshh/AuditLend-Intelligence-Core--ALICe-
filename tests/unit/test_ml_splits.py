...
def test_assign_time_split_uses_phase_two_boundaries():
    assert assign_time_split(date(2015, 12, 31)) == "train"
    assert assign_time_split(date(2016, 1, 1)) == "validation"
...
def test_split_rows_by_issue_date_groups_rows_consistently():
    rows = [
        {"loan_id": "1", "issue_date": date(2015, 5, 1)},
